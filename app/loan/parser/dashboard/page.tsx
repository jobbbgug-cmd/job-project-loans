...
export default function ParserDashboard() {
  ...
  useEffect(() => {
    try {
      ...
      if (stored) setCurrentRows(JSON.parse(stored) as Row[]);
    } catch { /* ignore */ }
    fetch('/api/loan/parser-sessions')
      .then(r => r.json())
  ...
  // ── Current session (localStorage) stats ────────────────────────────────
  const curSummaries   = currentRows.map(r => calcSummary(r));
  const curSumBet      = r2(currentRows.reduce((s, r) => s + (Number(r.betAmount) || 0), 0));
  const curSumResult   = r2(curSummaries.reduce<number>((s, v) => s + (v ?? 0), 0));
  const curProfit      = r2(curSumBet - curSumResult);
  const curWithResult  = currentRows.filter(r => r.result).length;
  const curWinCount    = currentRows.filter(r => r.result === 'win_full' || r.result === 'win_half').length;
  const curLoseCount   = currentRows.filter(r => r.result === 'lose_full' || r.result === 'lose_half').length;
  const curDrawCount   = currentRows.filter(r => r.result === 'draw').length;
  const curWinRate     = curWithResult > 0 ? Math.round((curWinCount / curWithResult) * 100) : 0;
  ...
  const hasAnyData     = hasCurrentData || hasSessions;

  // Decide what stats/rows to display in the top section (latest session only, not aggregate)
  const latestSession    = sessions[0];
  const displayRows      = hasCurrentData ? currentRows : safeRows(latestSession?.rows_data);
  const displaySummaries = hasCurrentData ? curSummaries : displayRows.map(r => calcSummary(r));
  const displayTotal     = hasCurrentData ? currentRows.length : displayRows.length;
  const displayBet       = hasCurrentData ? curSumBet : r2(Number(latestSession?.sum_bet ?? 0));
  const displayResult    = hasCurrentData ? curSumResult : r2(Number(latestSession?.sum_result ?? 0));
  const displayProfit    = hasCurrentData ? curProfit : r2(Number(latestSession?.profit ?? 0));
  const displayWithRes   = hasCurrentData ? curWithResult : displayRows.filter(r => r.result).length;
  const displayWinCount  = hasCurrentData ? curWinCount : displayRows.filter(r => r.result === 'win_full' || r.result === 'win_half').length;
  const displayLoseCount = hasCurrentData ? curLoseCount : displayRows.filter(r => r.result === 'lose_full' || r.result === 'lose_half').length;
  const displayDrawCount = hasCurrentData ? curDrawCount : displayRows.filter(r => r.result === 'draw').length;
  const displayWinRate   = hasCurrentData ? curWinRate : (displayWithRes > 0 ? Math.round((displayWinCount / displayWithRes) * 100) : 0);
  ...
  return (
    <div className="space-y-6 text-white">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            {hasCurrentData ? `ข้อมูลปัจจุบัน ${currentRows.length} รายการ` : `ข้อมูลรวมจาก ${sessions.length} session`}
          </p>
        </div>
        {!hasCurrentData && hasSessions && (
      ...
      {(() => {
        const allSessTotalRows = sessions.reduce((s, x) => s + safeRows(x.rows_data).length, 0) + (hasCurrentData ? currentRows.length : 0);
        const allSumBet    = r2(sessSumBet    + (hasCurrentData ? curSumBet    : 0));
        const allSumResult = r2(sessSumResult + (hasCurrentData ? curSumResult : 0));
        const allProfit    = r2(sessProfit    + (hasCurrentData ? curProfit    : 0));
        return (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard label="รายการทั้งหมด" value={String(allSessTotalRows)} sub="รวมทุกวัน" />
            <StatCard label="ยอดแทงรวม" value={allSumBet > 0 ? fmt(allSumBet) : '—'} sub="บาท (รวมทุกวัน)" />
            <StatCard
              label="ยอดผลสรุปรวม"
              value={allSumBet > 0 ? fmt(allSumResult) : '—'}
              sub="บาท (รวมทุกวัน)"
              color="text-yellow-400"
            />
            <StatCard
              label="กำไร / ขาดทุน"
              value={allSumBet > 0 ? (allProfit >= 0 ? `+${fmt(allProfit)}` : fmt(allProfit)) : '—'}
              sub="บาท (รวมทุกวัน)"
              color={allProfit > 0 ? 'text-emerald-400' : allProfit < 0 ? 'text-red-400' : 'text-slate-300'}
            />
          </div>
        );
      })()}
        ...
        <div className="lg:col-span-2 bg-slate-800 rounded-2xl border border-slate-700 overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-700">
            <p className="text-sm font-semibold text-white">
              {hasCurrentData ? `สรุปยอดรวมประจำวัน (ปัจจุบัน)` : `สรุปยอดรวมประจำวัน — ${latestSession?.label ?? ''}`}
            </p>
          </div>
      ...
      <div className="bg-slate-800 rounded-2xl border border-slate-700 overflow-hidden">
        ...
        {!hasSessions ? (
          <div className="px-5 py-8 text-center text-slate-500 text-sm">ยังไม่มีข้อมูลย้อนหลัง — กด "สรุปผลประจำวัน" ในหน้าแยกข้อมูล</div>
        ) : (
