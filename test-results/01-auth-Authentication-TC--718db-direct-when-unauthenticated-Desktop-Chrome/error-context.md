...
- Name: 01-auth.spec.ts >> Authentication >> TC-AUTH-02 protected page redirect when unauthenticated
- Location: tests/e2e/01-auth.spec.ts:17:7

# Error details

```
Error: page.goto: net::ERR_CONNECTION_REFUSED at http://localhost:3000/loan/dashboard
Call log:
  - navigating to "http://localhost:3000/loan/dashboard", waiting until "load"
...
> 19  |       await page.goto(path);
  ...
  113 |   test('TC-AUTH-10 authenticated user on login page still sees role badge in header', async ({ page }) => {
  114 |     await loginAsAdmin(page);
  115 |     await page.goto('/loan/login');
  116 |     // Header badge should still show because layout does not unmount on login route
  117 |     const badge = page.locator('header').getByText('แอดมิน');
  118 |     // Either badge is visible (layout renders) or page is a standalone login (badge absent)
  119 |     const badgeVisible = await badge.isVisible({ timeout: 3000 }).catch(() => false);
```