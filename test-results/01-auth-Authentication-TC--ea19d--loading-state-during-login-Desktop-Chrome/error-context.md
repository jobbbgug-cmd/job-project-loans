...
- Name: 01-auth.spec.ts >> Authentication >> TC-AUTH-08 submit button shows loading state during login
- Location: tests/e2e/01-auth.spec.ts:80:7
...
> 81  |     await page.goto('/loan/login');
  ...
  120 |     // Either outcome is acceptable — key assertion is no crash
  121 |     await expect(page.locator('body')).toBeVisible();
  122 |     expect(typeof badgeVisible).toBe('boolean');
  123 |   });
  124 | });
  125 | 
```