...
- Name: 02-navigation.spec.ts >> Navigation & RBAC >> TC-NAV-08 unauthenticated access to users page redirects to login
- Location: tests/e2e/02-navigation.spec.ts:101:7

# Error details

```
Error: page.goto: net::ERR_CONNECTION_REFUSED at http://localhost:3000/loan/users
Call log:
  - navigating to "http://localhost:3000/loan/users", waiting until "load"
...
```ts
  2   |  * TC-NAV-01 → TC-NAV-12
  3   |  * Navigation, sidebar, RBAC menu visibility
  4   |  */
  5   | import { test, expect, Page } from '@playwright/test';
  6   | import { loginAsAdmin, loginAs, ADMIN } from './helpers';
  7   | 
